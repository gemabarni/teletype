...
class HostPortalBindingComponent {
    ...
    if (creatingPortal || this.isSharing()) {
      ...
      return $.div({className: 'HostPortalComponent-connection-info'},
        ...
        $.div({className: 'HostPortalComponent-connection-info-heading ' + statusClassName},
          $.h1(null, 'Invite other EELs to join your swarm using this ID')
        ),
  ...
  async toggleShare () {
    console.log('EEEL SHARING TOGGLED');
    if (this.sharing) {
      // stop
      ...
      this.datid = await this.props.portalBindingManager.createHostPortalBinding()
      this.sharing = !this.sharing
